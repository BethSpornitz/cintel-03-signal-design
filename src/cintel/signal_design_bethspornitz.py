"""
signal_design_bethspornitz.py - Project script
...
Paths (relative to repo root)
    ...
    INPUT FILE: data/system_metrics_bethspornitz.csv
    OUTPUT FILE: artifacts/signals_bethspornitz.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.signal_design_bethspornitz
...
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "signals_bethspornitz.csv"
...
def main() -> None:
    ...
    # ----------------------------------------------------
    # STEP 2.6B: DEFINE A HIGH ERROR FLAG SIGNAL RECIPE
    # ----------------------------------------------------
    # This creates a label to help identify rows with
    # unusually high error rates.
    high_error_flag_signal_recipe: pl.Expr = (
        pl.when(is_requests_positive & ((pl.col("errors") / pl.col("requests")) > 0.02))
        .then(pl.lit("high_error"))
    ...
    LOG.info(
        "Created signal columns: error_rate, avg_latency_ms, throughput, error_flag"
    )
